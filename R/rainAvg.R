...
#' @param menu Defaults as true, setting to false speeds up process and bypasses the menu
#'
#' @return
#' @export
#'
#' @examples
#' #rainAvg(prop, rain)
rainAvg <- function(gaugeProp, rainAll, menu = TRUE){
  if(menu == TRUE){
    input<-NULL
    x<-NULL
    input <- menu(c("Yes", "No"),
    ...
    if (input == 1){
      ...
      if(dim(prop)[1] - (dim(rainAll)[2]-1) != 0){
